...
int main(int argc, char* argv[]) {
 ...
 //                                               
  ...
  //Workarround to a bug. Should not segfault
  //errno = zeEventPoolCreate(hDriver, &eventPoolDesc, 0, NULL, &hEventPool);
  ...
  zeCommandListAppendLaunchKernel(hCommandList, hKernel, &launchArgs, hEvent, 0, NULL);
  ...
  // The unit with 'GLOBA' give a too large number
  uint64_t  event_start;
  errno = zeEventGetTimestamp(hEvent, ZE_EVENT_TIMESTAMP_CONTEXT_START, &event_start);
  check_error(errno, "zeEventGetTimestamp");
 
  uint64_t  event_end;
  errno = zeEventGetTimestamp(hEvent, ZE_EVENT_TIMESTAMP_CONTEXT_END, &event_end);
  check_error(errno, "zeEventGetTimestamp");

  printf("Time on GPU: %lu clock \n",event_end-event_start);
  //   _                         
  //  /  |  _   _. ._  o ._   _  
  //  \_ | (/_ (_| | | | | | (_| 
  //                          _|
