...
int main(int argc, char* argv[]) {
   ...
   //
    ...
    cl_mem d_a = clCreateBuffer(context, CL_MEM_WRITE_ONLY, bytes, NULL, &err);
    ...
    const char *kernelstring =
        "__kernel void hello_world(__global int *a, int id) {"
        " for (int j = 0; j < id ; j++) {} "
        "atomic_xchg(a,id);"
        "}";
    ...
    err = clBuildProgram(program, device_count, devices, "-g -cl-opt-disable", NULL, NULL);
    if (err != CL_SUCCESS)
    {
