...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        cl::CommandQueue queue(context, devices[device_idx], CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE|CL_QUEUE_PROFILING_ENABLE);
        ...
        for(int id = 0; id < num_kernel; id++) {
            cl::Event event;
            if (id == 0) {
                event = kernel_func(cl::EnqueueArgs(queue, cl::NDRange(global_size), cl::NDRange(local_size)), id);
            } else {
                event = kernel_func(cl::EnqueueArgs(queue, events.back(), cl::NDRange(global_size), cl::NDRange(local_size)), id);
            }
        ...
        events.front().getProfilingInfo(CL_PROFILING_COMMAND_START, &time_start);
        events.back().getProfilingInfo(CL_PROFILING_COMMAND_END, &time_end);
        double nanoSeconds = time_end-time_start;
        std::cout << "OpenCl Execution time is: " << nanoSeconds * 1E-6 << " milliseconds" << std::endl;
    } catch (cl::Error e) {
