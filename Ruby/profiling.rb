...
queue = context.create_command_queue(dev, :properties => [OpenCL::CommandQueue::OUT_OF_ORDER_EXEC_MODE_ENABLE, OpenCL::CommandQueue::PROFILING_ENABLE] )
...
num_kernel.times { |id|
  ...
  event =
    if id == 0
      ...
      queue.enqueue_ndrange_kernel(kernel, [global_work_size], local_work_size: [local_work_size], :event_wait_list => [events.last])
    end
  events.push event
}
...
start = events.first.profiling_command_start
stop = events.last.profiling_command_end

puts "OpenCL execution time is: #{(stop - start) * 1e-6} milliseconds"
